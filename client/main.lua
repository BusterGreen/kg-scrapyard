...
RegisterNetEvent("KGCore:Client:OnPlayerLoaded", function()
    TriggerServerEvent("kg-scrapyard:server:LoadVehicleList")
end)
        ...
        for k,v in pairs(Config.Locations[i]) do
            if k ~= 'main' then
                if Config.UseTarget then
                    if k == 'deliver' then
                        exports["kg-target"]:AddBoxZone("yard"..i, v.coords, v.length, v.width, {
                            name = "yard"..i,
                            heading = v.heading,
                            minZ = v.coords.z - 1,
                            maxZ = v.coords.z + 1,
                        ...
                        exports["kg-target"]:AddBoxZone("list"..i, v.coords, v.length, v.width, {
                            name = "list"..i,
                            heading = v.heading,
                            minZ = v.coords.z - 1,
                            maxZ = v.coords.z + 1,
                    ...
                    scrapCombo:onPlayerInOut(function(isPointInside)
                        if isPointInside then
                            if not isBusy then
                                if k == 'deliver' then
                                    exports['kg-core']:DrawText(Lang:t('text.disassemble_vehicle'),'left')
                                else
                                    exports['kg-core']:DrawText(Lang:t('text.email_list'),'left')
                                end
                                KeyListener(k)
                            end
                        else
...
RegisterNetEvent('kg-scapyard:client:setNewVehicles', function(vehicleList)
    ...
    if Config.CurrentVehicles ~= nil and next(Config.CurrentVehicles) ~= nil then
        ...
        for k, v in pairs(Config.CurrentVehicles) do
            if Config.CurrentVehicles[k] ~= nil then
                local vehicleInfo = KGCore.Shared.Vehicles[v]
        ...
        SetTimeout(math.random(15000, 20000), function()
            emailSend = false
            TriggerServerEvent('kg-phone:server:sendNewMail', {
        ...
        KGCore.Functions.Notify(Lang:t('error.demolish_vehicle'), "error")
    end
end
        ...
        if not isBusy then
            if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
                if IsVehicleValid(GetEntityModel(vehicle)) then
                    local vehiclePlate = KGCore.Functions.GetPlate(vehicle)
                    KGCore.Functions.TriggerCallback('kg-scrapyard:checkOwnerVehicle',function(retval)
                        if retval then
                            isBusy = true
                            local scrapTime = math.random(28000, 37000)
                            ScrapVehicleAnim(scrapTime)
                            KGCore.Functions.Progressbar("scrap_vehicle", Lang:t('text.demolish_vehicle'), scrapTime, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                            }, {}, {}, {}, function() -- Done
                                TriggerServerEvent("kg-scrapyard:server:ScrapVehicle", GetVehicleKey(GetEntityModel(vehicle)))
                            ...
                            KGCore.Functions.Notify(Lang:t('error.smash_own'), "error")
                        end
                    end,vehiclePlate)
                else
                    KGCore.Functions.Notify(Lang:t('error.cannot_scrap'), "error")
                end
            else
                KGCore.Functions.Notify(Lang:t('error.not_driver'), "error")
            end
        end
    end
end
